...
Expr::register('sentinel::access-required', function($args)
{
    ...
    if ($data->is_banned) {
        Wind::reply([ 
            ...
            'error' => Strings::get('@messages.banned')
        ]);
    }
    ...
    if ($delta > 0) {
        Wind::reply([ 
            'response' => Wind::R_FORBIDDEN, 
            'error' => Strings::get('@messages.retry_at'), 
 ...
 * @code (`sentinel::access-denied` <identifier> [action='wait'] [timeout=15])
 * @example
...
Expr::register('sentinel::access-denied', function($args)
{
    ...
    if ($data->count_failed >= 3 || $action === 'block') {
        $data->count_failed = 0;
        $data->count_blocked++;
        $delay = 15 * pow(2, $data->count_blocked-1);
        $action = 'block';
    }
